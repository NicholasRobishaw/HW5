...
void Prefix_Trie::create_Trie(int nmers, bool subProblemB){
    ...
    segment = copy_From_Subject(randIndex, randIndex + segmentLength);
    string fragment = "";
    int index = 0;
...
void Prefix_Trie::search_From_Subject(string segment){
    int index;
    string currentFrag = "";
    ...
    for(index =0; index< segment.length() - FRAGMENT_SIZE; index++){
        // create fragmnet at index
        currentFrag = copy_String(segment, index, (index + (FRAGMENT_SIZE-1)));
        
        // call to search trie function
        search_Trie(currentFrag);
    }
}
    
        
// function that will search through trie ( GOTTA USE A STACK???)
void Prefix_Trie::search_Trie(string fragment){
    ...
    while(!stack.empty()){
        ...
        searchStack currentSearch = stack.top();
        
        // pop off value from stack
        stack.pop();
        
        if(currentSearch.fragmentIndex < FRAGMENT_SIZE){
    
            // if exists
            if(fragment[currentSearch.fragmentIndex] == 'A' && currentSearch.node->A != nullptr){
                // push it to the stack
                stack.push({currentSearch.node->A, currentSearch.fragmentIndex+1, currentSearch.mismatchScore});
            }
            else if(fragment[currentSearch.fragmentIndex] == 'C' && currentSearch.node->C != nullptr){
                // push it to the stack
                stack.push({currentSearch.node->C, currentSearch.fragmentIndex+1, currentSearch.mismatchScore});
            }
            else if(fragment[currentSearch.fragmentIndex] == 'G' && currentSearch.node->G != nullptr){
                // push it to the stack
                stack.push({currentSearch.node->G, currentSearch.fragmentIndex+1, currentSearch.mismatchScore});
            }
            else if(fragment[currentSearch.fragmentIndex] == 'T' && currentSearch.node->T != nullptr){
                // push it to the stack
                stack.push({currentSearch.node->T, currentSearch.fragmentIndex+1, currentSearch.mismatchScore});
            }
            else if(fragment[currentSearch.fragmentIndex] == 'N' && currentSearch.node->N != nullptr){
                // push it to the stack
                stack.push({currentSearch.node->N, currentSearch.fragmentIndex+1, currentSearch.mismatchScore});
            }
                
            // add 1 to mismatch, explore children ( possibly use mismatch index to come back and try another path)
            if(currentSearch.mismatchScore < 1){
                
                // try A
                if(fragment[currentSearch.fragmentIndex] != 'A' && currentSearch.node->A != nullptr){
                    // push it to the stack
                    stack.push({currentSearch.node->A, currentSearch.fragmentIndex+1, currentSearch.mismatchScore+1});
                }
                        
                // try C
                else if(fragment[currentSearch.fragmentIndex] != 'C' && currentSearch.node->C != nullptr){
                    // push it to the stack
                    stack.push({currentSearch.node->C, currentSearch.fragmentIndex+1, currentSearch.mismatchScore+1});
                }
                
                // try G
                else if(fragment[currentSearch.fragmentIndex] != 'G' && currentSearch.node->G != nullptr){
                    // push it to the stack
                    stack.push({currentSearch.node->G, currentSearch.fragmentIndex+1, currentSearch.mismatchScore+1});
                }
                
                // try T
                else if(fragment[currentSearch.fragmentIndex] != 'T' && currentSearch.node->T != nullptr){
                    // push it to the stack
                    stack.push({currentSearch.node->T, currentSearch.fragmentIndex+1, currentSearch.mismatchScore+1});
                }
                
                // try N
                else if(fragment[currentSearch.fragmentIndex] != 'N' && currentSearch.node->N != nullptr){
                    // push it to the stack
                    stack.push({currentSearch.node->N, currentSearch.fragmentIndex+1, currentSearch.mismatchScore+1});
                }
